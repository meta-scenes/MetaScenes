...
def run_simulation(config):
    ...
    ssg_path = config["ssg_path"]
    dataset_path = config["dataset_path"]
    obj_path_root = config["obj_path_root"]
    ...
    for one_scan in scan_list:
        ...
        invalid_obj_info[one_scan] = delect_objs

    # Save results
    output_file = os.path.join(obj_path_root, 'simu_matrix_info_support.json')
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(simu_matrix_info, f, ensure_ascii=False, indent=4)


    output_file_invalid = os.path.join(obj_path_root, 'invalid_objs.json')
...
def load_config(config_path='/home/huangyue/Mycodes/MetaScenes/scripts/physical_optimization/config.yaml'):
