...
def run(layout_type):
    ...
    output_file = os.path.join(save_path, f'{one_scan}.json')
...
if __name__ == '__main__':
    ...
    ssg_simu_path = '/mnt/fillipo/huangyue/recon_sim/11_release/SSG_simu/support/'
    layout_model_dir = '/mnt/fillipo/huangyue/recon_sim/3_recon/Layout/roomFormer_planes/'
    layout_heuristic_dir = '/home/huangyue/Mycodes/MetaScenes/scripts/layout_estimation/heuristic_layout'
    inst_name_dir = '/mnt/fillipo/scratch/masaccio/existing_datasets/scannet/scan_data/instance_id_to_name/'
    joint_matrix_all = json.load(
        open(f'{ssg_simu_path}/simu_matrix_info_support.json', 'rb'))
    invalid_objs = json.load(open(f'{ssg_simu_path}/invalid_objs.json', 'rb'))
    anno_info = json.load(open('/mnt/fillipo/huangyue/recon_sim/7_anno_v2/anno_info_ranking_v2.json', 'rb'))
    save_path = '/mnt/fillipo/huangyue/recon_sim/11_release/Physical_simu/'
    scan_list = ['scene0006_00', 'scene0001_00']
    layout_type = 'heuristic'
    ...
    for scan_idx, one_scan in enumerate(scan_list):
        ...
        if layout_type == 'model' and not os.path.exists(f'{layout_dir}/{one_scan}.json'):
            print ('Model layout construct fail, choose heuristic instead!')
            continue
